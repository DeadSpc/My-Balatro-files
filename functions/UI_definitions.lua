...
function G.UIDEF.settings_tab(tab)
  if tab == 'Game' then
    return {n=G.UIT.ROOT, config={align = "cm", padding = 0.05, colour = G.C.CLEAR}, nodes={
      create_option_cycle({label = localize('b_set_gamespeed'),scale = 0.8, options = {0.25, 0.5, 1, 2, 3, 4, 8, 16, 32}, opt_callback = 'change_gamespeed', current_option = (G.SETTINGS.GAMESPEED == 0.25 and 1 or G.SETTINGS.GAMESPEED == 0.5 and 2 or G.SETTINGS.GAMESPEED == 1 and 3 or G.SETTINGS.GAMESPEED == 2 and 4 or G.SETTINGS.GAMESPEED == 3 and 5 or G.SETTINGS.GAMESPEED == 4 and 6 or G.SETTINGS.GAMESPEED == 8 and 7 or G.SETTINGS.GAMESPEED == 16 and 8 or G.SETTINGS.GAMESPEED == 32 and 9 or 1)}),
    ...
    {n=G.UIT.C, config={align = "cm", padding = 0.15}, nodes={
      ...
      {n=G.UIT.R, config={align = "cm", padding = 0.1, r=0.2, colour = G.C.BLACK}, nodes={
        ...
        {n=G.UIT.C, config={align = "cm", padding = 0.15}, nodes={
          ...
          UIBox_button({button = 'your_collection_alchemicals', label = {"Карты Алхимии"}, count = G.DISCOVER_TALLIES.alchemicals, minw = 4, id = 'your_collection_alchemicals', colour = G.C.Alchemy}),
...
function create_UIBox_your_collection_tarots()
  ...
  local tarotch = 0
  if not (math.fmod(#G.P_CENTER_POOLS.Tarot, 8) == 0) then
    tarotch = math.floor(#G.P_CENTER_POOLS.Tarot/8) + 1
  else tarotch = math.floor(#G.P_CENTER_POOLS.Tarot/8) end  
  local tarot_options = {}
  for i = 1, tarotch do
    table.insert(tarot_options, localize('k_page')..' '..tostring(i)..'/'..tostring(tarotch))
  end

    for j = 1, #G.your_collection do
      for i = 1, 4 do
      local center = G.P_CENTER_POOLS["Tarot"][(j-1) * 4 + i]
      if type(center) == "table" then 
        local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w/2, G.your_collection[j].T.y, G.CARD_W, G.CARD_H, nil, center)
        card:start_materialize(nil, i>1 or j>1)
        G.your_collection[j]:emplace(card)
...
function create_UIBox_your_collection_spectrals()
  ...
  local spectraltch = 0
  if not (math.fmod(#G.P_CENTER_POOLS.Spectral, 8) == 0) then
    spectraltch = math.floor(#G.P_CENTER_POOLS.Spectral/8) + 1
  else spectraltch = math.floor(#G.P_CENTER_POOLS.Spectral/8) end
  local spectral_options = {}
  for i = 1, spectraltch do
    table.insert(spectral_options, localize('k_page')..' '..tostring(i)..'/'..tostring(spectraltch))
  end

    for j = 1, #G.your_collection do
      for i = 1, 4 do
      local center = G.P_CENTER_POOLS["Spectral"][(j-1) * 4 + i] 
    ...
    {n=G.UIT.C, config={align = "cm", r = 0.1, colour = G.C.BLACK, padding = 0.1, emboss = 0.05}, nodes={
      {n=G.UIT.C, config={align = "cm"}, nodes={
        {n=G.UIT.R, config={align = "cm"}, nodes={
          ...
          {n=G.UIT.R, config={align = "cm"}, nodes=tag_matrix[5]},
        }}
      }} 
    }}  
  }})
