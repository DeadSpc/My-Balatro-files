...
function add_round_eval_row(config)
	...
	local delayScale = 4
	--Change delay scale to edit speed, default is 4 (4 times faster)
    ...
    if config.name ~= 'bottom' then
        ...
        delay(0.2 / delayScale)

        G.E_MANAGER:add_event(Event({
            trigger = 'before',delay = 0.5 / delayScale,
            func = function()
        ...
        if num_dollars > 60 then 
            ...
            G.E_MANAGER:add_event(Event({
                trigger = 'before',delay = 0.38 / delayScale,
                func = function()
            ...
            for i = 1, num_dollars or 1 do
                G.E_MANAGER:add_event(Event({
                    trigger = 'before',delay = (0.18/ delayScale) - ((num_dollars > 20 and 0.13) or (num_dollars > 9 and 0.1) or 0),
                    func = function()
                        if i%30 == 1 then 
                            G.round_eval:add_child(
                                {n=G.UIT.R, config={align = "cm", id = 'dollar_row_'..(dollar_row+1)..'_'..config.name}, nodes={}},
        ...
        delay(0.4 / delayScale)
        G.E_MANAGER:add_event(Event({
            trigger = 'before',delay = 0.6 / delayScale,
            func = function()
                UIBox{
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Spectral' then 
        ...
        if _c.name == 'Incense' then info_queue[#info_queue+1] = G.P_CENTERS.e_negative; loc_vars = {_c.config.extra.dollars, _c.config.extra.increase} end
    ...
    elseif _c.set == 'Tarot' then
       ...
       elseif _c.name == "The Seeker" then loc_vars = {_c.config.alchemicals} 
